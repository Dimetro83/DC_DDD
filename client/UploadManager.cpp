...
void UploadManager::on(TimerManagerListener::Second, uint64_t aTick) noexcept {
	...
	if(!isFireball) {
		...
		if(!isFireball && !isFileServer) {
			if(	((time(NULL) - Util::getStartTime()) > 12 * 60 * 60) && // > 12 hours
				(Socket::getTotalUp() > 20ULL * 1024 * 1024 * 1024) && // > 20 GiB uploaded
				(ShareManager::getInstance()->getSharedSize() > 500 * 1024 * 1024 * 1024)) // > 500 GiB shared
			{ 
					isFileServer = true;
					ClientManager::getInstance()->infoUpdated();
			}
