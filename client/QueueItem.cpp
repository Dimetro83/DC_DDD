...
uint64_t QueueItem::getDownloadedBytes() const {
	...
	for(auto i = downloads.begin(); i != downloads.end(); ++i) {
		total += (*i)->getPos();// [!] crash-strong-full-r6555.dmp
	}
...
void QueueItem::getChunksVisualisation(int type,vector<Segment>& p_segments) const    // type: 0 - downloaded bytes, 1 - running chunks, 2 - done chunks
{
	p_segments.clear();
	switch (type)
	{
