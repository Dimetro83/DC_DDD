...
void WebServerManager::Start(){
	...
	m_pages["/"] = new WebPageInfo(INDEX, "");
	m_pages["/index.htm"] = new WebPageInfo(INDEX, "");	
	m_pages["/dlqueue.html"] = new WebPageInfo(DOWNLOAD_QUEUE, "Download Queue");
	m_pages["/dlfinished.html"] = new WebPageInfo(DOWNLOAD_FINISHED, "Finished Downloads");
	m_pages["/ulqueue.html"] = new WebPageInfo(UPLOAD_QUEUE, "Upload Queue");
	m_pages["/ulfinished.html"] = new WebPageInfo(UPLOAD_FINISHED, "Finished Uploads");
	m_pages["/weblog.html"] = new WebPageInfo(LOG, "Logs");
	m_pages["/syslog.html"] = new WebPageInfo(SYSLOG, "System Logs");
	m_pages["/search.html"] = new WebPageInfo(SEARCH, "Search");
	m_pages["/logout.html"] = new WebPageInfo(LOGOUT, "Logout");
...
void WebServerManager::Stop() {
	...
	for(WebPages::iterator p = m_pages.begin(); p != m_pages.end(); ++p){
			delete p->second;
			p->second = NULL;
	}
...
string WebServerManager::getLoginPage(){
	...
	pagehtml += "<head>";
	pagehtml += "    	<title>DDD++ webserver - Login Page</title>";   
	...
	pagehtml += "	<link rel='stylesheet' href='http://strongdc.sf.net/webserver/strong.css' type='text/css' title='Default style' media='screen' />";
    pagehtml += "</head>";
    pagehtml += "<body>";
    pagehtml += "<div id='index_obsah'>";
    pagehtml += "<h1>DDD++ Webserver</h1>";
...
string WebServerManager::getPage(const string& file, const string& IP) {
	...
	WebPages::const_iterator find = m_pages.find(file);
	if(find != m_pages.end()) page = find->second;	
    pagehtml = "<!DOCTYPE html PUBLIC '-//W3C//DTD XHTML 1.1//EN' 'http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd'>";
    pagehtml += "<html xmlns='http://www.w3.org/1999/xhtml' xml:lang='cs'>";
    pagehtml += "<head>";
    pagehtml += "    <title>DDD++ webserver</title>";
	...
	switch(page->id){
		case INDEX:
			pagehtml+="Welcome to the DDD++ WebServer";
			break;
