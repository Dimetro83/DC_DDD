...
namespace dcpp {
const int64_t MIN_BLOCK_SIZE = 65536;
...
public:
	 ...
	 */
			...
			fileSize(p_FileSize), blockSize(p_BlockSize)
		{
			...
			dcassert(p_DataSize == l_n * Hasher::BYTES);
			...
			{
				const size_t l_next_offset = l_offset + Hasher::BYTES;
				 leaves.push_back(MerkleValue(p_Data + l_offset)); 
	...
	static uint16_t calcBlocks(int64_t aFileSize){
		...
		//[+]PPA
		static int64_t getMaxBlockSize(int64_t p_file_size)
		{
		    const int64_t l_result = max(calcBlockSize(p_file_size, 10), MIN_BLOCK_SIZE);
		    return l_result;
		}
...
private:	
	...
	MerkleValue getHash(int64_t start, int64_t length) {
		...
		if(length <= blockSize) {
			...
			if(start < (int64_t)leaves.size()) //[+]PPA
			   return leaves[(uint32_t)(start)];
			else
