...
public:
    ...
    static TCHAR* Util::strstr(const TCHAR *str1, const TCHAR *str2, int *pnIdxFound); //[+]PPA
...
//	static int64_t getDataFromInet(LPCWSTR agent, const DWORD frameBufferSize, string const& url, boost::shared_array<unsigned char>& dataOut, LONG timeOut = 0, IDateReceiveReporter* reporter = NULL );
