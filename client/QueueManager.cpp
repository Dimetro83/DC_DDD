	...
	do {
		...
		if(i != userQueue[p].end()) {
			...
			for(auto j = i->second.begin(); j != i->second.end(); ++j) {
				...
				if(qi->getDownloads().size() && qi->getDownloads().front()->getType() == Transfer::TYPE_TREE) {
					continue;
				}
				if(!qi->isSet(QueueItem::FLAG_USER_LIST)) {
					int64_t blockSize = HashManager::getInstance()->getBlockSize(qi->getTTH());
...
int QueueManager::Rechecker::run() {
	while(true) {
		...
		TigerTree tt;
		bool gotTree = CFlylinkDBManager::getInstance()->getTree(tth, tt);
...
bool QueueManager::addSource(QueueItem* qi, const HintedUser& aUser, Flags::MaskType addBad) throw(QueueException, FileException) {
    if(!aUser.user) //[+]PPA http://flylinkdc.blogspot.com/2009/11/crash-strong-r3154.html
		throw QueueException("[C++Bug] aUser.user = NULL! Target:" + Util::getFileName(qi->getTarget()));
...
uint8_t QueueManager::FileQueue::getMaxSegments(int64_t filesize) const {
	...
	if(BOOLSETTING(SEGMENTS_MANUAL)) {
		MaxSegments = min((uint8_t)SETTING(NUMBER_OF_SEGMENTS), (uint8_t)200);
	} else {
...
Download* QueueManager::getDownload(UserConnection& aSource, string& aMessage) noexcept {
	...
	if(q->getDownloadedBytes() > 0) { // [!] crash-strong-full-r6555.dmp
...
void QueueManager::processList(const string& name, const HintedUser& user, int flags) {
	...
	if(flags & QueueItem::FLAG_DIRECTORY_DOWNLOAD) {
		...
		for(auto i = dl.begin(); i != dl.end(); ++i) {
			DirectoryItem* di = *i;
			dirList.download(di->getName(), di->getTarget(), false); //todo di->getName() IS NULL
			delete di;
		}
	}
	if(flags & QueueItem::FLAG_MATCH_QUEUE) {
		dirList.log_matched_files(user,matchListing(dirList));
	}
}
...
void QueueLoader::startTag(const string& name, StringPairList& attribs, bool simple) {
	...
	if(!inDownloads && name == "Downloads") {
		...
		if(cur == NULL && name == sDownload) {
			...
			ClientManager::getInstance()->updateNick(user, l_nick);
			user->setFirstNick(l_nick);
			try {
	...
	for(auto i = queue.begin(); i != queue.end(); ++i) {
		...
		for(QueueItem::SourceConstIter j = sources.begin(); j != sources.end(); ++j) {
			if(	(*j).isSet(QueueItem::Source::FLAG_PARTIAL) && 
				(*j).getPartialSource() && //[+]PPA
				(*j).getPartialSource()->getNextQueryTime() <= now &&
