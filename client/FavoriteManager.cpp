...
void FavoriteManager::save() {
	...
	try {
		...
		for(FavoriteHubEntryList::const_iterator i = favoriteHubs.begin(), iend = favoriteHubs.end(); i != iend; ++i) {
			...
			xml.addChildAttrib("ISPsync", (*i)->getISPsync());
		}
...
bool FavoriteManager::load_from_url()
{
	...
	int l_size = Util::getDataFromInet(_T("FavoriteManager::load_from_url"), 4096, l_url, l_data, 100);
	if(l_size == 0)
	{
...
void FavoriteManager::load() {
	...
	if(load_from_url() && m_sync_hub_external.size())
	{
		for(auto m = m_sync_hub_external.cbegin(); m !=m_sync_hub_external.cend() && m_sync_hub_local.size(); ++m)
...
void FavoriteManager::load(SimpleXML& aXml, bool p_is_url /* = false*/) {
	dontSave = true;
	bool needSave = false;
	...
	if(aXml.findChild("Hubs")) {
		...
		while(aXml.findChild("Hub")) {
			...
			e->setName(l_Name);
			const bool l_connect_empty = aXml.getChildAttrib("Connect").empty();
			bool l_connect = aXml.getBoolChildAttrib("Connect");
			...
			const bool l_ISPsync = aXml.getBoolChildAttrib("ISPsync");
			e->setISPsync(l_ISPsync);
			...
			else
			{
				...
				l_HubEntry->setISPsync(true);
				const bool l_first_ISP = l_HubEntry->getGroup() != "ISP";
				if(l_connect_empty && l_first_ISP)
				  l_HubEntry->setConnect(true);
				if(!l_connect_empty)
				  l_HubEntry->setConnect(l_connect);
