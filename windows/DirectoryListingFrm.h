	...
	public:
		...
		const tstring getText(uint8_t col) const {
			switch(col) {
				...
				case COLUMN_PATH: 
					{
				     tstring l_result =  type == FILE ? Text::toT(ShareManager::getInstance()->toRealPath(file->getTTH())) : Util::emptyStringT;
		             if(type == FILE && l_result.empty())
		              if(file->isSet(DirectoryListing::FLAG_DOWNLOAD))
			            l_result = _T("Файл уже качал...");
		              else
		              if(file->isSet(DirectoryListing::FLAG_OLD_TTH))
			            l_result = _T("Этот файл был у меня в шаре...");
                      return l_result;
					}
				case COLUMN_TTH: return type == FILE ? Text::toT(file->getTTH().toBase32()) : Util::emptyStringT;
				case COLUMN_HIT:  
					    if((type == FILE && file->getHit()) || (type == DIRECTORY && dir->getTotalHit())) 
							return type == FILE ? Util::toStringW(file->getHit()) : Util::toStringW(dir->getTotalHit());
						break;
				case COLUMN_TS: 
					{
				      if(type == FILE && file->getTS())		
						  return Text::toT(Util::formatTime("%d-%m-%Y %H:%M", file->getTS())); 
					  else
                       if(type == DIRECTORY) //[!] fix r349
                         if(const uint32_t l_TS = dir->getTotalTS())
 	                      return Text::toT(Util::formatTime("%d-%m-%Y %H:%M",l_TS ));
					}
				    break; 
				case COLUMN_BITRATE: 
				...
				case COLUMN_MEDIA_XY: 
					if(type == FILE && file->m_media.m_mediaX && file->m_media.m_mediaY)
						return Text::toT(Util::toString(file->m_media.m_mediaX) + "x" + Util::toString(file->m_media.m_mediaY));
					break;
				case COLUMN_MEDIA_VIDEO: 
					if(type == FILE)
					  return Text::toT(file->m_media.m_video);
					break;
				case COLUMN_MEDIA_AUDIO: 
					if(type == FILE)
					return Text::toT(file->m_media.m_audio);
					break;
		...
		static int compareItems(const ItemInfo* a, const ItemInfo* b, uint8_t col) {
			if(a->type == DIRECTORY) {
				if(b->type == DIRECTORY) {
					...
					case COLUMN_HIT:  return compare(a->dir->getTotalHit(), b->dir->getTotalHit());
					case COLUMN_TS:  return compare(a->dir->getTotalTS(), b->dir->getTotalTS());
				    case COLUMN_BITRATE: return compare(a->dir->getTotalBitrate(), b->dir->getTotalBitrate()); 
				...
				case COLUMN_HIT:  return compare(a->file->getHit(), b->file->getHit());
				case COLUMN_TS:  return compare(a->file->getTS(), b->file->getTS());
				case COLUMN_BITRATE: return compare(a->file->m_media.m_bitrate, b->file->m_media.m_bitrate); 
	...
	int run() {
		try {
			if(!mFile.empty()) {
                ...
                const bool l_list = ( _strnicmp(filename.c_str(), "files", 5) // !SMT!-UI
                 || _strnicmp(filename.c_str()+filename.length()-8, ".xml.bz2", 8));
				mWindow->dl->loadFile(mFile,_stricmp(filename.c_str(), "files.xml.bz2")==0);
...
/**
 * @file
 * $Id: DirectoryListingFrm.h 568 2011-07-24 18:28:43Z bigmuscle $
 */
