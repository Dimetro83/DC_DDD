...
int TransferView::columnIndexes[] = { COLUMN_USER, COLUMN_HUB, COLUMN_STATUS, COLUMN_TIMELEFT, COLUMN_SPEED, COLUMN_FILE, COLUMN_SIZE, COLUMN_PATH, COLUMN_CIPHER, COLUMN_IP
#ifdef PPA_INCLUDE_COLUMN_RATIO
, COLUMN_RATIO 
#endif
};
int TransferView::columnSizes[] = { 150, 150, 250, 75, 75, 175, 100, 200, 100, 150
#ifdef PPA_INCLUDE_COLUMN_RATIO
, 50 
#endif
};
...
void TransferView::ItemInfo::removeAll() {
	if(hits <= 1) {
		...
		if(!BOOLSETTING(CONFIRM_DELETE) || ::MessageBox(0, _T("Do you really want to remove this item?"), _T(APPNAME) _T(" ") T_VERSIONSTRING, MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2) == IDYES)
...
LRESULT TransferView::onCustomDraw(int /*idCtrl*/, LPNMHDR pnmh, BOOL& bHandled) {
	...
	case CDDS_SUBITEM | CDDS_ITEMPREPAINT: {
		...
		if((colIndex == COLUMN_STATUS) && (ii->status != ItemInfo::STATUS_WAITING)) {
			...
			ctrlTransfers.GetItemText((int)cd->nmcd.dwItemSpec, cd->iSubItem, l_buf, 255);
			if(l_buf[0] != 0) {
				rc.left += 2;
...
void TransferView::ItemInfo::update(const UpdateInfo& ui) {
	...
	if(ui.updateMask & UpdateInfo::MASK_IP) {
		...
		if(BOOLSETTING(ENABLE_LAST_IP) && !ui.IP.empty())
			ClientManager::getInstance()->store_last_ip(ui.user,Text::fromT(ui.IP));
		ip = ui.IP;
	}
...
void TransferView::starting(UpdateInfo* ui, const Transfer* t) {
	...
	if(country.empty()) {
		ui->setIP(Text::toT(ip), 0);
		  ...
		  const int l_ind = WinUtil::getFlagImage(country);
	  	  ui->setIP(Text::toT(ip), l_ind);
	}
}
