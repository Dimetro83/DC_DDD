...
LRESULT DetectionEntryDlg::onAdd(WORD , WORD , HWND , BOOL& ) {
	...
	if(dlg.DoModal() == IDOK) {
		if(ctrlParams.find(Text::toT(dlg.name)) == -1) {
			...
			::MessageBox(m_hWnd, CTSTRING(PARAM_EXISTS), _T(APPNAME) _T(" ") T_VERSIONSTRING, MB_OK);
		}
	}
	return 0;
}
...
#else
		::MessageBox(m_hWnd, _T("matchExp(formattedExp, test).c_str()"), CTSTRING(REGEXP_TESTER), MB_ICONINFORMATION | MB_OK);
#endif
