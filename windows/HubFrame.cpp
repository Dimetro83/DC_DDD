...
int HubFrame::columnSizes[] = { 100, 75, 75, 75, 100, 75, 100, 100, 100,50,50, 50, 40, 40, 40, 300 };
int HubFrame::columnIndexes[] = { OnlineUser::COLUMN_NICK, OnlineUser::COLUMN_SHARED, OnlineUser::COLUMN_EXACT_SHARED,
	OnlineUser::COLUMN_DESCRIPTION, OnlineUser::COLUMN_TAG,	OnlineUser::COLUMN_CONNECTION, OnlineUser::COLUMN_IP, 
	OnlineUser::COLUMN_LAST_IP, 
	OnlineUser::COLUMN_UPLOAD,
	OnlineUser::COLUMN_DOWNLOAD,
...
ResourceManager::Strings HubFrame::columnNames[] = { ResourceManager::NICK, ResourceManager::SHARED, ResourceManager::EXACT_SHARED, 
	ResourceManager::DESCRIPTION, ResourceManager::TAG, ResourceManager::CONNECTION, 
	ResourceManager::IP_BARE, 
	ResourceManager::IP_BARE_LAST, 
    ResourceManager::U_RATIO,
    ResourceManager::D_RATIO,
...
LRESULT HubFrame::onDoubleClickUsers(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/) {
	...
	if(item && item->iItem != -1 && (ctrlUsers.getItemData(item->iItem)->getUser() != ClientManager::getInstance()->getMe())) {
			...
			if ( (GetKeyState(VK_MENU) & 0x8000) ||	( ((GetKeyState(VK_CONTROL) & 0x8000) == 0) ^ (BOOLSETTING(USE_CTRL_FOR_LINE_HISTORY) == true) ) ) {
				...
				if (curCommandPosition > 0) {
					...
					if (curCommandPosition == prevCommands.size()) {
						...
						if(ctrlMessage.GetWindowTextLength())
...
bool HubFrame::matchFilter(const OnlineUser& ui, int sel, bool doSizeCompare, FilterModes mode, int64_t size) {
...
#else
	{
			...
			{
				for(uint8_t i = OnlineUser::COLUMN_FIRST; i < OnlineUser::COLUMN_LAST; ++i) 
					if(insert = Text::is_sub_tstring(ui.getText(i).c_str(),filter))
				     break;
			} 
			else 
				insert = Text::is_sub_tstring(ui.getText(static_cast<uint8_t>(sel)),filter);
	}
#endif
...
bool HubFrame::PreparePopupMenu(CWindow* /*pCtrl*/, OMenu& menu) {
	...
	if(!isMe) {
		menu.AppendMenu(MF_STRING, IDC_PUBLIC_MESSAGE, CTSTRING(SEND_PUBLIC_MESSAGE));		//	Послать ЛС
		...
		if(count == 1) {
			...
			if(!FavoriteManager::getInstance()->isIgnoredUser(ou->getUser()->getCID())) {		//	Игнор
	...
	if(count == 1) {
		if(BOOLSETTING(LOG_PRIVATE_CHAT)) {		//	Открыть лог
...
LRESULT HubFrame::onCustomDraw(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/) {
	...
	case CDDS_ITEMPREPAINT: {
			if(OnlineUser* ui = (OnlineUser*)cd->nmcd.lItemlParam)
			{
			if(ui->getIdentity().isOp()) {
				cd->clrText = SETTING(OP_COLOR);
			} else if (FavoriteManager::getInstance()->isFavoriteUser(ui->getUser())) {
...
case CDDS_SUBITEM | CDDS_ITEMPREPAINT: {
	    const bool l_is_last_ip = ctrlUsers.findColumn(cd->iSubItem) == OnlineUser::COLUMN_LAST_IP;
		if(BOOLSETTING(GET_USER_COUNTRY) && 
			  ((ctrlUsers.findColumn(cd->iSubItem) == OnlineUser::COLUMN_IP) || l_is_last_ip ) ) {
			...
			ctrlUsers.GetItemText((int)cd->nmcd.dwItemSpec, cd->iSubItem, l_buf, 255);
			if(l_buf[0] != 0) {
				rc.left += 2;
				...
				POINT p = { rc.left, top };
				const string l_ip = l_is_last_ip ? ou->getUser()->getLastIP() : ou->getIdentity().getIp();
				string tmpCountry = Util::getIpCountry(l_ip);
				 // Only attempt to grab a country mapping if we actually have an IP address
				 ou->m_flagImage = WinUtil::getFlagImage(tmpCountry);
				 if(ou->m_flagImage >0 )
			       WinUtil::flagImages.Draw(cd->nmcd.hdc, ou->m_flagImage, p, LVSIL_SMALL);
				 top = rc.top + (rc.Height() - WinUtil::getTextHeight(cd->nmcd.hdc) - 1)/2;
				if(ou->m_flagImage >= WinUtil::flagImageCount)
				{
					wstring l_w_full_name = Text::toT(tmpCountry);
				    l_w_full_name += _T(" (") + Text::toT(l_ip) + _T(")");
				    ::ExtTextOut(cd->nmcd.hdc, rc.left + 30, top + 1, ETO_CLIPPED, rc, l_w_full_name.c_str(), l_w_full_name.length(), NULL);
				}
				else
				    ::ExtTextOut(cd->nmcd.hdc, rc.left + 30, top + 1, ETO_CLIPPED, rc, l_buf, _tcslen(l_buf), NULL);
...
void HubFrame::on(UserReport, const Client*, const Identity& i) noexcept
{
...
/**
 * @file
 * $Id: HubFrame.cpp 568 2011-07-24 18:28:43Z bigmuscle $
 */