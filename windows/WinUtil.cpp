...
void WinUtil::init(HWND hWnd) {
	...
	CMenuHandle dcsite;
	dcsite.CreatePopupMenu();

	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE1, CTSTRING(MENU_SITE1));
	dcsite.AppendMenu(MF_SEPARATOR);
	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE2, CTSTRING(MENU_SITE2));
	dcsite.AppendMenu(MF_SEPARATOR);
	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE3, CTSTRING(MENU_SITE3));
	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE4, CTSTRING(MENU_SITE4));
	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE5, CTSTRING(MENU_SITE5));
	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE6, CTSTRING(MENU_SITE6));
	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE7, CTSTRING(MENU_SITE7));
	dcsite.AppendMenu(MF_SEPARATOR);
/*	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE8, CTSTRING(MENU_SITE9));
	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE9, CTSTRING(MENU_SITE9));
	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE10, CTSTRING(MENU_SITE10));
	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE11, CTSTRING(MENU_SITE11));
	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE12, CTSTRING(MENU_SITE12));
	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE13, CTSTRING(MENU_SITE13));
	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE14, CTSTRING(MENU_SITE14));
*/	dcsite.AppendMenu(MF_STRING, IDC_DCSITE_SITE15, CTSTRING(MENU_SITE15));

	mainMenu.AppendMenu(MF_POPUP, (UINT_PTR)(HMENU)dcsite, CTSTRING(MENU_DCSITE));
	...
	help.AppendMenu(MF_STRING, IDC_UPDATE, CTSTRING(UPDATE_CHECK));

	help.AppendMenu(MF_SEPARATOR);
	help.AppendMenu(MF_STRING, ID_APP_ABOUT, CTSTRING(MENU_ABOUT));
/*	help.AppendMenu(MF_STRING, IDC_HELP_SOURCE_CODE, CTSTRING(MENU_SOURCE_CODE));
...
/** @todo fix this so that the system icon is used for dirs as well (we need
	...
	if (!g_isWineCheck) //[+]PPA под линуксом пока падаем http://flylinkdc.blogspot.com/2010/08/customlocationsbmp-wine.html
	{
        if (UserLocations.m_hBitmap = (HBITMAP)::LoadImage(NULL, Text::toT(Util::getPath(Util::PATH_USER_CONFIG) + "CustomLocations.bmp").c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE)) 
...
void WinUtil::decodeFont(const tstring& setting, LOGFONT &dest) {
	...
	if(!face.empty()) {
		...
		// V512	A call of the 'memset' function will lead to underflow of the buffer 'dest.lfFaceName'.	flylinkdc	winutil.cpp	845	False
...
#define LINE2 _T("-- http://ddd-dc.ru/  <DDD++ ") T_VERSIONSTRING _T(" / ") _T(DCVERSIONSTRING) _T(">")
TCHAR *msgs[] = { _T("\r\n-- I'm a happy DDD++ user. You could be happy too.\r\n") LINE2};
...
bool WinUtil::checkCommand(tstring& cmd, tstring& param, tstring& message, tstring& status, bool& thirdPerson) {
	...
	if(stricmp(cmd.c_str(), _T("log")) == 0) {
	...
	} else if(stricmp(cmd.c_str(), _T("DDD++")) == 0) {
		...
		if (!param.empty()) {
			...
			WinUtil::openLink(_T("http://") + Text::toT(Util::encodeURI(Text::fromT(param))));
		}
...
void WinUtil::registerMagnetHandler() {
	...
	TCHAR l_buf[512];
	tstring openCmd;
	tstring appName = _T("\"") + Text::toT(getAppName()) + _T("\"");
	l_buf[0] = 0;
	...
	if(::RegOpenKeyEx(HKEY_CURRENT_USER, _T("SOFTWARE\\Classes\\magnet\\shell\\open\\command"), 0, KEY_READ, &hk) == ERROR_SUCCESS) {
		DWORD bufLen = sizeof(l_buf);
		::RegQueryValueEx(hk, NULL, NULL, NULL, (LPBYTE)l_buf, &bufLen);
		::RegCloseKey(hk);
	}
	openCmd = l_buf;
	l_buf[0] = 0;
	...
	if (strnicmp(aUrl.c_str(), _T("magnet:?"), 8) == 0) {
		...
		for(TStringList::const_iterator idx = mag.getTokens().begin(); idx != mag.getTokens().end(); ++idx) {
			...
			if(param.length() == 85 && strnicmp(param.c_str(), _T("urn:bitprint:"), 13) == 0) {
				...
				if(!param.empty() && param[param.size()-1] == '"') //[+] FlylinkDCfix http://code.google.com/p/flylinkdc/issues/detail?id=68&q=magnet
					fname = param.substr(0,param.size()-1);
				else
					fname = param;
...
int WinUtil::getIconIndex(const tstring& aFileName) {
	if(BOOLSETTING(USE_SYSTEM_ICONS)) {
		...
		const DWORD_PTR l_Result = SHGetFileInfo(fn.c_str(), FILE_ATTRIBUTE_NORMAL, &fi, sizeof(fi), SHGFI_ICON | SHGFI_SMALLICON | SHGFI_USEFILEATTRIBUTES);
		if(l_Result)
		{
			fileImages.AddIcon(fi.hIcon);
			::DestroyIcon(fi.hIcon);
...
uint8_t WinUtil::getFlagIndexByCode(const char* countryCode) {
	...
	while(begin <= end) {
		...
		const int cmp = memcmp(countryCode, countryCodes[mid], 2);
...
int WinUtil::getFlagImage(string& country)
{
	// !SMT!-IP
	if (!country.empty())
	if (isdigit(country[0]))
	{
      ...
      return getFlagIndexByCode(country.c_str());
  return 0;
}
...
#ifdef FLY_INCLUDE_CZCHARS_DISABLE
wchar_t arrayutf[42] = { L'Б', L'И', L'П', L'Й', L'М', L'Н', L'ј', L'Т', L'У', L'Ш', L'Љ', L'Ќ', L'Ъ', L'Щ', L'Э', L'Ћ', L'б', L'и', L'п', L'й', L'м', L'н', L'ѕ', L'т', L'у', L'ш', L'љ', L'ќ', L'ъ', L'щ', L'э', L'ћ', L'Д', L'Л', L'Ц', L'Ь', L'д', L'л', L'ц', L'ь', L'Ј', L'і' };
...
string WinUtil::generateStats() {
	if(LOBYTE(LOWORD(GetVersion())) >= 5) {
		...
		snprintf(buf, sizeof(buf), "\n-=[ DDD++ %s ]=-\r\n-=[ Uptime: %s][ Cpu time: %s ]=-\r\n-=[ Memory usage (peak): %s (%s) ]=-\r\n-=[ Virtual memory usage (peak): %s (%s) ]=-\r\n-=[ Downloaded: %s ][ Uploaded: %s ]=-\r\n-=[ Total download: %s ][ Total upload: %s ]=-\r\n-=[ System Uptime: %s]=-", 
			A_VERSIONSTRING, formatTime(time(NULL) - Util::getStartTime()).c_str(), Text::fromT(Util::formatSeconds((kernelTime + userTime) / (10I64 * 1000I64 * 1000I64))).c_str(), 
...
/**
 * @file
 * $Id: WinUtil.cpp 578 2011-10-04 14:27:51Z bigmuscle $
 */
