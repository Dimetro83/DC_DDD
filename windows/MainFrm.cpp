...
public:
	...
	int run() {
		for(StringIter i = files.begin(); i != files.end(); ++i) {
			...
			try {
				dl.loadFile(*i);
		            dl.log_matched_files(user,QueueManager::getInstance()->matchListing(dl));
...
LRESULT MainFrame::OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled) {
	...
	SetWindowText(_T(APPNAME) _T(" ") T_VERSIONSTRING);
	...
	m_CmdBar.m_arrCommand.Add(IDC_OPEN_DOWNLOADS);			// -Damaks- [#] Кнопка "Открыть папку загрузки"
	...
	m_CmdBar.m_arrCommand.Add(IDC_REFRESH_FILE_LIST);			// -Damaks- [+] Кнопка "Обновить шару"
	m_CmdBar.m_arrCommand.Add(IDC_OPEN_MY_LIST);			// -Damaks- [+] Кнопка "Посмотреть свой список файлов"
	m_CmdBar.m_arrCommand.Add(ID_FILE_QUICK_CONNECT);			// -Damaks- [+] Кнопка "Быстрое соединение"
...
//#ifdef FLYLINKDC_USE_DISCOVER
//	trayMenu.AppendMenu(MF_STRING, IDC_FLYLINK_DISCOVER, CTSTRING(MENU_IDC_FLYLINK_DISCOVER));
//#endif
	trayMenu.AppendMenu(MF_SEPARATOR);
	trayMenu.AppendMenu(MF_STRING, IDC_REFRESH_FILE_LIST, CTSTRING(MENU_REFRESH_FILE_LIST));
	trayMenu.AppendMenu(MF_STRING, IDC_REFRESH_FILE_LIST_PURGE, CTSTRING(MENU_REFRESH_FILE_LIST_PURGE));
	trayMenu.AppendMenu(MF_SEPARATOR);
	...
	m_normalicon = (HICON)::LoadImage(GetModuleHandle(NULL), MAKEINTRESOURCE(IDR_MAINFRAME), IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR);
	m_pmicon = (HICON)::LoadImage(GetModuleHandle(NULL), MAKEINTRESOURCE(IDR_TRAY_PM), IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR);
...
HWND MainFrame::createToolbar() {
	if(!tbarcreated) {
		if(SETTING(TOOLBARIMAGE) == "")
			...
			largeImages.CreateFromImage(Text::toT(SETTING(TOOLBARIMAGE)).c_str(), 32, 0, CLR_DEFAULT, IMAGE_BITMAP, LR_CREATEDIBSECTION | LR_SHARED | LR_LOADFROMFILE);
		...
		if(SETTING(TOOLBARHOTIMAGE) == "")
			...
			largeImagesHot.CreateFromImage(Text::toT(SETTING(TOOLBARHOTIMAGE)).c_str(), 32, 0, CLR_DEFAULT, IMAGE_BITMAP, LR_CREATEDIBSECTION | LR_SHARED | LR_LOADFROMFILE);
	...
	const int buttonsCount = sizeof(ToolbarButtons) / sizeof(ToolbarButtons[0]);
		...
		if(xml.findChild("Version")) {
			if(Util::toDouble(xml.getChildData()) > VERSIONFLOAT) {
				...
				if(xml.findChild("VeryOldVersion")) {
					if(Util::toDouble(xml.getChildData()) >= VERSIONFLOAT) {
						string msg = xml.getChildAttrib("Message", "Your version of DDD++ contains a serious bug that affects all users of the DC network or the security of your computer.");
...
if(add) {
		if(bTrayIcon == false) {
			...
			_tcsncpy(nid.szTip, _T(APPNAME), 127);
			nid.szTip[63] = '\0';
...
LRESULT MainFrame::onLink(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	bool isFile = false;
	switch(wID) {
		case IDC_DCSITE_SITE1: site = _T("http://ddd-dc.ru/help-dc"); break;
		case IDC_DCSITE_SITE2: site = _T("http://dc.yaroslavl.biz"); break;
		case IDC_DCSITE_SITE3: site = _T("http://ddd-dc.ru/hublist-svao"); break;
		case IDC_DCSITE_SITE4: site = _T("http://ddd-dc.ru/hublist-home"); break;
		case IDC_DCSITE_SITE5: site = _T("http://ddd-dc.ru/hublist-netbynet"); break;
		case IDC_DCSITE_SITE6: site = _T("http://ddd-dc.ru/hublist-triolan"); break;
		case IDC_DCSITE_SITE7: site = _T("http://ddd-dc.ru/hublist-akadoural"); break;
/*		case IDC_DCSITE_SITE8: site = _T("http://ddd-dc.ru/"); break;
		case IDC_DCSITE_SITE9: site = _T("http://ddd-dc.ru/"); break;
		case IDC_DCSITE_SITE10: site = _T("http://ddd-dc.ru/"); break;
		case IDC_DCSITE_SITE11: site = _T("http://ddd-dc.ru/"); break;
		case IDC_DCSITE_SITE12: site = _T("http://ddd-dc.ru/"); break;
		case IDC_DCSITE_SITE13: site = _T("http://ddd-dc.ru/"); break;
		case IDC_DCSITE_SITE14: site = _T("http://ddd-dc.ru/"); break;
*/		case IDC_DCSITE_SITE15: site = _T("http://ddd-dc.ru/cooperation"); break;
		case IDC_HELP_HOMEPAGE: site = _T("http://ddd-dc.ru/"); break;
/*		case IDC_HELP_FAQ: site = _T("http://strongdc.sf.net/forum/viewtopic.php?t=4067"); break;
		case IDC_HELP_DISCUSS: site = _T("http://mydc.ru/topic4706.html"); break;
		case IDC_HELP_GEOIPFILE: site = _T("http://www.maxmind.com/download/geoip/database/GeoIPCountryCSV.zip"); break;

		case IDC_HELP_SOURCE_CODE: site = _T("http://code.google.com/p/flylinkdc/"); break;

		case IDC_HELP_TRANSLATIONS: site = _T("http://strongdc.sf.net/forum/viewtopic.php?t=5201"); break;
		case IDC_HELP_FAQ: site = _T("http://strongdc.sf.net/forum/viewtopic.php?t=4067"); break;
		case IDC_HELP_DISCUSS: site = _T("http://groups.google.com/group/flylinkdc/"); break;
*/		default: dcassert(0);
	}
...
LRESULT MainFrame::onRefreshFileListPurge(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	LogManager::getInstance()->message("Purge TTH database...");
	return 0;
}
...
LRESULT MainFrame::onTrayIcon(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/) {
	if (lParam == WM_LBUTTONUP) {
		...
		_tcsncpy(nid.szTip, (_T("D: ") + Util::formatBytesW(DownloadManager::getInstance()->getRunningAverage()) + _T("/s (") + 
			...
			+ _T("\r\nUptime: ") + Util::formatSeconds(time(NULL) - Util::getStartTime())).c_str(), 63);
...
void MainFrame::on(QueueManagerListener::Finished, const QueueItem* qi, const string& dir, const Download* download) noexcept {
	if(qi->isSet(QueueItem::FLAG_CLIENT_VIEW)) {
		if(qi->isSet(QueueItem::FLAG_USER_LIST)) {
			...
			DirectoryListInfo* i = new DirectoryListInfo(qi->getDownloads().front()->getHintedUser(), Text::toT(qi->getListName()), Text::toT(dir), static_cast<int64_t>(download->getAverageSpeed()));
