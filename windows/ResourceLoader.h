/*
 * Copyright (C) 2006-2010 Crise, crise<at>mail.berlios.de
 *
...
class ExCImage : public CImage, public FastAlloc<ExCImage>, public intrusive_ptr_base<ExCImage>, boost::noncopyable
{
	public:
		...
		ExCImage(UINT id, LPCTSTR pType = RT_RCDATA, HMODULE hInst = NULL) noexcept
		{
			m_hBuffer = NULL;
			LoadFromResource(id, pType, hInst);
		}
		ExCImage(UINT id, UINT type, HMODULE hInst = NULL) noexcept
		{
			m_hBuffer = NULL;
		...
		bool LoadFromResource(UINT id, LPCTSTR pType = RT_RCDATA, HMODULE hInst = NULL, bool useDefaultHINST = false) noexcept;
...
inline void ExCImage::Destroy() noexcept
{
	CImage::Destroy();
	if (m_hBuffer)
	{
...
inline bool ExCImage::LoadFromResource(UINT id, LPCTSTR pType, HMODULE hInst, bool useDefaultHINST) noexcept
{
	HRSRC hResource = ::FindResource(hInst, MAKEINTRESOURCE(id), pType);
	if (!hResource)
	{
		hInst = NULL; // [!] SSA - try to search in original resource
	...
	public:
		...
		static void LoadImageList(UINT id, CImageList& aImgLst, int cx, int cy)
		{
			...
			try
			{
				...
				int imageCount = (img.GetWidth() / cy);
