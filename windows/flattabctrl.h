...
public:
	...
	void setDirty(HWND aWnd) {
		TabInfo* ti = getTabInfo(aWnd);
		dcassert(ti != NULL);
		if(!ti) //[+]PPA "crash-strong-stack-(r1729).dmp"
			return;
...
public:
	...
	// For buggy Aero MDI (Vista), since it doesn't look like MS is gonna fix it... idea taken from MSDN
	...
	void createHubTab(bool mini) {
		if(!created) {
			dcassert(getTab());
			getTab()->addTab(m_hWnd, C, I, I_state, mini);
			created = true;
		}
	}
private:
