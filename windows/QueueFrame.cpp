...
const tstring QueueFrame::QueueItemInfo::getText(int col) const {
	switch(col) {
		...
		case COLUMN_STATUS: {
			const size_t l_online = QueueManager::getInstance()->countOnlineUsers(qi);
...
void QueueFrame::removeSelected() {
	if(!BOOLSETTING(CONFIRM_DELETE) || MessageBox(CTSTRING(REALLY_REMOVE), _T(APPNAME) _T(" ") T_VERSIONSTRING, MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2) == IDYES)
...
LRESULT QueueFrame::onCustomDraw(int /*idCtrl*/, LPNMHDR pnmh, BOOL& bHandled) {
	...
	case CDDS_ITEMPREPAINT:
		{
			if(qii && qii->getQueueItem())
			if(!qii->getQueueItem()->getBadSources().empty()) {
				// 2012-05-13_06-39-45_2CPJL6BOLY2N76PCOYCXJO7BHU35CXL7JYQS62A_186C39A4_crash-strong-stack-r9957-x64.dmp
