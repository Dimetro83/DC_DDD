...
public:
	...
	LRESULT onCollapsed(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
		...
		storeIP = (m_ctrlStoreIP.GetCheck() == 1);
		return 0;
	}
	...
	static const TStringSet& getLastSearches() { return lastSearches; }
	...
	public:
		...
		SearchInfo(const SearchResultPtr& aSR) : sr(aSR), collapsed(true), parent(NULL), flagIndex(0), hits(0) { 
			if (!sr->getIP().empty()) {
				...
				if(!tmpCountry.empty()) 
				    flagIndex = WinUtil::getFlagImage(tmpCountry);
			}
		}
			...
			switch(col) {
				...
				case COLUMN_LOCAL_PATH:
					...
					if(sr->getType() == SearchResult::TYPE_FILE) {
						...
						if(l_result.empty())
						{
//	TODO			     if(QueueManager::getInstance()->isQueueItem(sr->getTTH()))
// 						   l_result = _T("Файл в очереди..."); 
//						else
						if(CFlylinkDBManager::getInstance()->is_download_tth(sr->getTTH()))
  						   l_result = _T("Я скачивал этот файл..."); 
						else
				       if(CFlylinkDBManager::getInstance()->is_old_tth(sr->getTTH()))
				...
				case COLUMN_IP: {
					...
					if (!ip.empty()) {
						// Only attempt to grab a country mapping if we actually have an IP address
						//[-]PPA string tmpCountry = Util::getIpCountry(ip);
                        //[-]PPA WinUtil::getFlagIndex(tmpCountry.c_str());
						//[-]PPA if(!tmpCountry.empty()) {
						//[-]PPA 	ip = tmpCountry + " (" + ip + ")";
						//[-]PPA}
					}
					return Text::toT(ip);
				}
	...
	static TStringSet lastSearches;
...
/**
 * @file
 * $Id: SearchFrm.h 568 2011-07-24 18:28:43Z bigmuscle $
 */
