...
{
public:
	TypedListViewCtrl() : sortColumn(-1), sortAscending(true), hBrBg(WinUtil::bgBrush), leftMargin(0),m_count_update_info(0)  { }
	...
	LRESULT onGetDispInfo(int /* idCtrl */, LPNMHDR pnmh, BOOL& /* bHandled */) {
		...
		if(di && di->item.lParam && //[+]PPA
			di->item.iSubItem >= 0 && //[+]PPA 
	...
	int getCountUpdateInfo() const	{ return m_count_update_info;}
