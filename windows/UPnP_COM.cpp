...
bool UPnP_COM::init()
{
	...
	if(!FAILED(CLSIDFromString(upnps, &upnp)))
	  {
	IID iupnp;
	OLECHAR iupnps[] = L"{B171C812-CC76-485A-94D8-B6B3A2794E99}";
	       if(!FAILED(CLSIDFromString(iupnps, &iupnp)))
		   {
	pUN = 0;
 		   CoInitialize(NULL);
	...
	if(FAILED(hr))
		     {
		pUN = 0;
				 LogManager::getInstance()->message("UPnP_COM Error CoCreateInstance");
}
		   }
	else
		LogManager::getInstance()->message("UPnP_COM Error CLSIDFromString B171C812-CC76-485A-94D8-B6B3A2794E99");
	}
	else
		LogManager::getInstance()->message("UPnP_COM Error CLSIDFromString AE1E00AA-3FD5-403C-8A27-2BBDC30CD0E1");
  return pUN != 0;
}
