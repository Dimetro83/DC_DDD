...
int SearchFrame::columnSizes[] = { 210, 80, 100, 50, 80, 100,100, 40, 70, 150, 80, 100, 150 };
...
static ResourceManager::Strings columnNames[] = { ResourceManager::FILE,  ResourceManager::HIT_COUNT, ResourceManager::USER, ResourceManager::TYPE, ResourceManager::SIZE,
	ResourceManager::PATH, ResourceManager::LOCAL_PATH, ResourceManager::SLOTS, ResourceManager::CONNECTION, 
...
LRESULT SearchFrame::onCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
{
	...
	m_ctrlStoreIP.SetFont(WinUtil::systemFont, FALSE);
...
void SearchFrame::onTab(bool shift) {
	HWND wnds[] = {
		...
		ctrlFiletype.m_hWnd, ctrlSlots.m_hWnd, ctrlCollapsed.m_hWnd, m_ctrlStoreIP.m_hWnd, ctrlDoSearch.m_hWnd, ctrlSearch.m_hWnd, 
		ctrlResults.m_hWnd
	};
...
void SearchFrame::addSearchResult(SearchInfo* si) {
			const SearchResultPtr& sr = si->sr;
	        if(storeIP && BOOLSETTING(ENABLE_LAST_IP))
	        {
		     CFlylinkDBManager::getInstance()->store_last_ip(
			   sr->getHubURL(),
			   Text::fromT(WinUtil::getNicks(sr->getUser()->getCID(),sr->getHubURL())),
			   sr->getIP()
			  ); 
              sr->getUser()->setLastIP(sr->getIP());
	         }
...
LRESULT SearchFrame::onCustomDraw(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/) {
	...
	case CDDS_ITEMPREPAINT: {
		...
		if(si->sr) {
			...
			if(si->sr->m_is_tth_download)
               cd->clrTextBk = SETTING(DUPE_COLOR)/2;
			else
			if(si->sr->m_is_tth_remembrance)
               cd->clrTextBk = SETTING(DUPE_COLOR)/3;
		}
	...
	case CDDS_SUBITEM | CDDS_ITEMPREPAINT: {
		if(BOOLSETTING(GET_USER_COUNTRY) && (ctrlResults.findColumn(cd->iSubItem) == COLUMN_IP)) {
			...
			ctrlResults.GetItemText((int)cd->nmcd.dwItemSpec, cd->iSubItem, l_buf, 255);
			if(l_buf[0] != 0) {
				rc.left += 2;
...
//+BugMaster: new options; small optimization
    unsigned sz = ctrlSearchBox.GetWindowTextLength();
	if (sz == 39) {
		TCHAR buf[40];
		ctrlSearchBox.GetWindowText(buf, 40);
		for(int i = 0; i < 39; i++)
	...
	{ // [+]FlylinkDC++ http://code.google.com/p/flylinkdc/issues/detail?id=155
		LocalArray<TCHAR, 1000> l_buf;
		ctrlSearchBox.GetWindowText(l_buf, 1000);
		string l_tth = Text::fromT(l_buf.data());
		if(l_tth.find("magnet:?") != string::npos)
	{
				const string::size_type l_xl = l_tth.find("&xl=");
				if (l_xl != string::npos
					&& l_tth.size()-l_xl > 39 //[!]FlylinkDC++ Team. fix crash: "magnet:?xt=urn:tree:tiger:&xl=3451326464&dn=sr-r3ts12.iso"
							)
					{
						l_tth = l_tth.substr(l_xl-39,39);
						ctrlSearchBox.SetWindowText(Text::toT(l_tth).c_str());
		                ctrlFiletype.SetCurSel(ctrlFiletype.FindStringExact(0, _T("TTH")));
                                m_lastFindTTH = true; 
				return 0;
				    }
...
/**
 * @file
 * $Id: SearchFrm.cpp 568 2011-07-24 18:28:43Z bigmuscle $
 */
