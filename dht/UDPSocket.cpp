	...
	void UDPSocket::checkIncoming() throw(SocketException)
	{
		if(socket->wait(delay, Socket::WAIT_READ) == Socket::WAIT_READ)
		{
			...
			if(len > 1)
			{
				...
				boost::scoped_array<uint8_t> destBuf(new uint8_t[destLen]); //17662	V554	Incorrect use of unique_ptr. The memory allocated with 'new []' will be cleaned using 'delete'.
	...
	const uint64_t now = GET_TICK();
		...
		if(packet.get())
		{
			try
			{
				...
				boost::scoped_array<uint8_t> data(new uint8_t[length]); // 17662	V554	Incorrect use of unique_ptr. The memory allocated with 'new []' will be cleaned using 'delete'.
