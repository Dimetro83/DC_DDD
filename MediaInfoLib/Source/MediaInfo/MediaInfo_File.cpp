...
bool MediaInfo_Internal::SelectFromExtension (const String &Parser)
{
    ...
    // Audio
    #if defined(MEDIAINFO_AAC_YES)
        else if (Parser==_T("Aac"))         {Info=new File_Aac(); ((File_Aac*)Info)->Mode=File_Aac::Mode_ADIF;}//-V517
    #endif
    #if defined(MEDIAINFO_AAC_YES)
    ...
    #endif
    #if defined(MEDIAINFO_MPC_YES)
        else if (Parser==_T("Mpc"))         Info=new File_Mpc();//-V517
    #endif
    #if defined(MEDIAINFO_MPCSV8_YES)
...
/*    #if defined(MEDIAINFO_AU_YES)
    ...
    #endif
*/ //[!]FlylinkDC PVS V517	The use of 'if (A) {...} else if (A) {...}' pattern was detected. There is a probability of logical error presence. Check lines: 492,528.
    #if defined(MEDIAINFO_RKAU_YES)
