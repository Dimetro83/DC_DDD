    ...
    while (Buffer_Offset+16<=Buffer_Size)
    {
        if ((ByteSize==0 || ByteSize==4) && ((Buffer_TotalBytes+Buffer_Offset)%4)==0)
        {
            if (Buffer[Buffer_Offset  ]==0xF8
             ...
             && Buffer[Buffer_Offset+3]==0x1F) //SMPTE 337M 16-bit, BE // https://www.box.net/shared/ad9d15ec22f26216d7fd
            {
                ByteSize=4;
                Container_Bits=16;
                Stream_Bits=16;
        ...
        if ((ByteSize==0 || ByteSize==8) && ((Buffer_TotalBytes+Buffer_Offset)%8)==0)
        {
            if (Buffer[Buffer_Offset  ]==0x00
             ...
             && Buffer[Buffer_Offset+7]==0x1F) //16-bit in 32-bit, BE  // [crash count = 1] https://www.box.net/shared/ce5c3fffc806edb6a5c7
            {
                ByteSize=8;
                Container_Bits=32;
                Stream_Bits=16;
