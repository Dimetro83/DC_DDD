    ...
    else
    {
        ...
        if ((Event_Generic->EventCode&0x00FFFF03)==((MediaInfo_Event_Global_Demux<<8)|0x03)) //Demux version 3
        {
            ...
            else for (size_t Pos=0; Pos<Event->StreamIDs_Size; Pos++)
            {
                ...
                else if (Event->StreamIDs_Width[Pos] /* && Event->StreamIDs_Width[Pos] */ ) //[!]PVS-Studio V501 There are identical sub-expressions 'Event->StreamIDs_Width [Pos]' to the left and to the right of the '&&' operator.	MediaInfoLib	mediainfo_config_mediainfo.cpp	804	False	
                {
                    Ztring ID;
