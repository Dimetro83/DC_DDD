...
void File_Avc::Data_Parse()
{
    ...
    if (Element_Size)
    {
        while (Element_Size && Buffer[Buffer_Offset+(size_t)Element_Size-1]==0) // https://www.box.net/shared/5fb9cd7f7b906ca2e1f5
            Element_Size--;
    }
...
void File_Avc::slice_header()
{
    ...
    FILLING_BEGIN_PRECISE();
        ...
        if ((*seq_parameter_set_Item)->pic_order_cnt_type!=1 && first_mb_in_slice==0 && Element_Code!=0x14) //Not slice_layer_extension
        {
			...
			if(TemporalReferences_Offset_pic_order_cnt_lsb_Last < TemporalReferences.size()) //[+]FlylinkDC++
			{
...
void File_Avc::sei_message_buffering_period(int32u &seq_parameter_set_id)
{
    ...
    std::vector<seq_parameter_set_struct*>::iterator seq_parameter_set_Item;
    if (seq_parameter_set_id>=seq_parameter_sets.size() || (*(seq_parameter_set_Item=seq_parameter_sets.begin()+seq_parameter_set_id))==NULL || !(*seq_parameter_set_Item)->IsSynched) //TODO Crash
    {
        //Not yet present
        Skip_BS(Data_BS_Remain(),                               "Data (seq_parameter_set is missing)");
        BS_End();
